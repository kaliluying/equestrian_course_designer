...
class FeedbackIndexView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            monthly_income = (
                current_month_orders.aggregate(Sum("amount"))["amount__sum"] or 0
            )
            ...
            premium_users = UserProfile.objects.filter(
                is_premium=True, premium_expire_date__gt=today
            ).count()

            # 获取上月会员数量
            last_month_premium_users = UserProfile.objects.filter(
                is_premium=True, premium_expire_date__gt=last_month_start
            ).count()
            ...
            if last_month_premium_users > 0:
                premium_trend = round(
                    (
                        (premium_users - last_month_premium_users)
                        / last_month_premium_users
                    )
                    * 100,
                    2,
                )
            ...
            for plan in membership_plans:
                plan_users = UserProfile.objects.filter(
                    membership_plan=plan, premium_expire_date__gt=today
                ).count()
                if plan_users > 0:
                    membership_distribution.append(
                        {"name": plan.name, "value": plan_users}
                    )

            # 获取支付渠道分布数据
            payment_channels_data = (
                MembershipOrder.objects.filter(status="paid")
            ...
            for channel in payment_channels_data:
                ...
                payment_channels.append(
                    {"name": channel_name, "value": channel["count"]}
                )

            # 获取最近10个订单
            recent_orders = MembershipOrder.objects.all().order_by("-created_at")[:10]
